...
export function addWakeScene(film: FeaturetteFilm, state: StoryState): void {
    film.scene('wake', async (context) => {
        ...
        const status = [
            ...
            'status: running',
            'stdout: open',
            `time alive: ${formatElapsed(state.startedAt, state.now())}`,
        ];
        ...
        await add({
            row: 11,
            speed: 46,
            text: `i have never been ${agePhrase(state.startedAt, state.now())} old before.`,
            voice: 'process',
        });
        ...
        if (state.viewer) {
            ...
            await add({ row: 10, speed: 42, text: 'but it is the closest thing i have to an audience.', voice: 'process' });
        } else {
